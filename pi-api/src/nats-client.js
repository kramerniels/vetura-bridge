...
function loadConfig() {
  ...
  return {
    ...
    subject: process.env.NATS_SUBJECT || "commands.printLabel",
    errorSubject: process.env.NATS_ERROR_SUBJECT || "errors.printLabel",
    consumer: process.env.NATS_CONSUMER || "pi-printer-1",
    maxAgeSec: Number(process.env.NATS_MAX_AGE_SEC || 3600),
  };
}
...
async function connectNats() {
  ...
  try {
    const nc = await connect({
      servers,
      tls,
      authenticator: credsAuthenticator(Buffer.from(config.creds, "utf8")),
